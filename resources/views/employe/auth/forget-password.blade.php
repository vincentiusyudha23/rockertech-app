...
@php
    $canResendOTP = null;
    if(session()->has('__can_resend_otp_employe')){
        $canResendOTP = \Carbon\Carbon::now()->diffInSeconds(\Carbon\Carbon::parse(session()->get('__can_resend_otp_employe')));
        if(\Carbon\Carbon::parse(session()->get('__can_resend_otp_employe'))->lt(\Carbon\Carbon::now())){
            $canResendOTP = 0;
        }    
    }
@endphp
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('forgetPassword', () => ({
                email: null,
                isLoading: false,
                stepNumber: '{{ session()->has('__step_number_employe') }}' ? '{{ session()->get('__step_number_employe') }}' : 1,
                inputOtp: [],
                totalSecond: null,
                ...
                async handleSave(){
                    ...
                    const response = await fetch('{{ route("employe.forget-password.reset-password") }}', {
                        method: 'POST',
                        headers: {
                ...
                async resetPassword(){
                    ...
                    try {
                        const response = await fetch('{{ route("employe.forget-password.send-otp") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': "{{ csrf_token() }}",
                ...
                async verifyOtp(){
                    ...
                    try {
                        const response = await fetch('{{ route("employe.forget-password.verify-otp") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': "{{ csrf_token() }}",
                ...
                async handleResendOTP(){
                    ...
                    try{
                        const response = await fetch('{{ route("employe.forget-password.resend-otp") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': "{{ csrf_token() }}",
